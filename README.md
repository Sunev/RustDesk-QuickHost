...
* Fixed Excel clipboard format synchronization between the controller and host sides.
  * Added support for Excel's native `XML Spreadsheet` clipboard format.
  * Improved preservation of cell formatting, including font size, background color, text color, bold, underline, and other formatting attributes.
  * Improved compatibility when copying and pasting multi-row and multi-column cell ranges.
  * Fixed an issue where cell line breaks could be lost during clipboard synchronization.

* Improved the About dialog layout.
  * Increased the available content area to prevent bottom text from being clipped.

* Updated application and file version information to `v1.1.2.4`.
...
The repository includes `build.ps1`, a PowerShell build script for compiling the project with the MSVC toolchain.

### Usage

```powershell
.\build.ps1 [-Configuration Debug|Release] [-Platform x86|x64] [-OutputDir path] [-DryRun] [-Help]
```

| Parameter      | Description                                                                 |
|----------------|-----------------------------------------------------------------------------|
| `-Configuration` | Build configuration. `Release` (default) or `Debug`.                      |
| `-Platform`      | Target platform. `x86` (default) or `x64`.                                |
| `-OutputDir`     | Custom output directory. Defaults to `build\{configuration}-{platform}`.  |
| `-DryRun`        | Show the commands that would be executed without actually running them.    |
| `-Help`          | Display usage information.                                                 |

Examples:

```powershell
# Build Release x86 (default)
.\build.ps1

# Build Debug x64
.\build.ps1 -Configuration Debug -Platform x64

# Build Release x86 with a custom output directory
.\build.ps1 -OutputDir D:\MyBuild

# Preview the build commands without executing
.\build.ps1 -DryRun
```

### How It Works

The build script automates the entire compilation process in several steps:

1. **Zstd source preparation** — If the zstd source code is not present under `third_party\zstd`, the script automatically clones the `v1.5.7` tag from the official zstd repository. It then applies a Windows XP compatibility patch (`third_party\zstd-winxp.diff`) to ensure the zstd library can be used on Windows XP.

2. **Vcpkg bootstrap** — If vcpkg is not yet set up under `third_party\vcpkg`, the script clones the vcpkg repository and bootstraps it. Vcpkg is used to manage third-party C/C++ dependencies.

3. **Dependency installation** — The script detects whether the required headers (`libyuv/convert.h`, `vpx/vpx_encoder.h`, `sodium.h`, `zstd.h`) are available. If any are missing, it uses vcpkg to install `libyuv`, `libvpx`, and `libsodium` with the appropriate triplet. For x86 builds, a custom `x86-windows-static-v141xp` triplet is used to maintain Windows XP compatibility.

4. **Zstd static library build** — The script builds the zstd static library (`zstd_static.lib`) using CMake with the MSVC generator. For x86 builds, it sets `CMAKE_GENERATOR_PLATFORM=Win32` and adds the `_WIN32_WINNT=0x0501` / `WINVER=0x0501` compile flags to target Windows XP.

5. **Main source compilation** — The C++ source files (`main.cpp`, `portable_host.cpp`, `xp_fls_compat.cpp`) and the resource file (`portable_host.rc`) are compiled with `cl.exe`. For x86 builds, the script attempts to use the `v141_xp` toolset (via `-vcvars_ver=14.16`) to produce XP-compatible binaries.

6. **Linking** — All object files and the compiled resource are linked into the final executable `RustDeskQS.exe`. For x86 (XP-compatible) builds, critical linker flags are applied:
   - `/subsystem:windows,5.01` — Sets the Windows subsystem version to 5.01 (Windows XP).
   - `/DYNAMICBASE:NO` — Disables ASLR (not supported on Windows XP).
   - `/NXCOMPAT:NO` — Disables the DEP compatibility flag (not supported on Windows XP).

7. **Cleanup** — In Release mode, intermediate `.obj` and `.res` files are automatically removed after a successful build.

### Prerequisites

- **Visual Studio** (2017, 2019, 2022, or later) with the **Desktop development with C++** workload, or **Visual Studio Build Tools**.
- **Git** — Required for cloning zstd source code and bootstrapping vcpkg.
- **CMake** — Required for building the zstd static library.

For Windows XP-compatible x86 builds, the **v141_xp toolset** (available in Visual Studio 2017 or 2019) must be installed. The script will automatically detect and use it when building with `-Platform x86`.

### Environment Variables

If third-party libraries are installed in non-standard locations, set the following environment variables before running the script:

- `LIBYUV_ROOT` — Path to the libyuv installation root.
- `VPX_ROOT` — Path to the libvpx installation root.
- `SODIUM_ROOT` — Path to the libsodium installation root.
- `ZSTD_ROOT` — Path to the zstd installation root.
...
* 修正 Excel 在主控端與被控端之間的剪貼簿格式同步問題。
  * 新增支援 Excel 原生 `XML Spreadsheet` 剪貼簿格式。
  * 改善儲存格格式保留，包括字號、背景顏色、文字顏色、粗體、下劃線及其他格式屬性。
  * 改善多列、多欄儲存格範圍的跨端複製與貼上相容性。
  * 修正剪貼簿同步時，儲存格換行可能遺失的問題。

* 改善「關於」視窗版面。
  * 增加內容顯示空間，避免底部文字被裁切。

* 更新程式與檔案版本資訊至 `v1.1.2.4`。
...
本儲存庫包含 `build.ps1`，這是一個使用 MSVC 工具鏈編譯專案的 PowerShell 建置腳本。
...
| 參數             | 說明                                                              |
|------------------|-------------------------------------------------------------------|
| `-Configuration` | 建置組態。`Release`（預設）或 `Debug`。                           |
| `-Platform`      | 目標平台。`x86`（預設）或 `x64`。                                 |
| `-OutputDir`     | 自訂輸出目錄。預設為 `build\{configuration}-{platform}`。         |
| `-DryRun`        | 僅顯示將執行的命令，不實際執行。                                  |
| `-Help`          | 顯示使用說明。                                                    |
...
建置腳本會自動化整個編譯流程，分為以下幾個步驟：

1. **Zstd 原始碼準備** — 如果 `third_party\zstd` 下沒有 zstd 原始碼，腳本會自動從官方 zstd 儲存庫克隆 `v1.5.7` 標籤。接著套用 Windows XP 相容性修補檔（`third_party\zstd-winxp.diff`），確保 zstd 函式庫可在 Windows XP 上使用。

2. **Vcpkg 初始化** — 如果 `third_party\vcpkg` 尚未設定 vcpkg，腳本會克隆 vcpkg 儲存庫並執行初始化。Vcpkg 用於管理第三方 C/C++ 相依套件。

3. **相依套件安裝** — 腳本會偵測必要的標頭檔（`libyuv/convert.h`、`vpx/vpx_encoder.h`、`sodium.h`、`zstd.h`）是否可用。若有缺少，則使用 vcpkg 安裝 `libyuv`、`libvpx` 和 `libsodium`，並使用適當的 triplet。對於 x86 建置，會使用自訂的 `x86-windows-static-v141xp` triplet 以維持 Windows XP 相容性。

4. **Zstd 靜態函式庫建置** — 腳本使用 CMake 搭配 MSVC 產生器來建置 zstd 靜態函式庫（`zstd_static.lib`）。對於 x86 建置，會設定 `CMAKE_GENERATOR_PLATFORM=Win32` 並加入 `_WIN32_WINNT=0x0501` / `WINVER=0x0501` 編譯旗標以鎖定 Windows XP。

5. **主要原始碼編譯** — C++ 原始檔（`main.cpp`、`portable_host.cpp`、`xp_fls_compat.cpp`）和資源檔（`portable_host.rc`）會使用 `cl.exe` 進行編譯。對於 x86 建置，腳本會嘗試使用 `v141_xp` 工具集（透過 `-vcvars_ver=14.16`）來產生 XP 相容的二進位檔。

6. **連結** — 所有目的檔和已編譯的資源會被連結成最終執行檔 `RustDeskQS.exe`。對於 x86（XP 相容）建置，會套用以下關鍵連結器旗標：
   - `/subsystem:windows,5.01` — 將 Windows 子系統版本設為 5.01（Windows XP）。
   - `/DYNAMICBASE:NO` — 停用 ASLR（Windows XP 不支援）。
   - `/NXCOMPAT:NO` — 停用 DEP 相容性旗標（Windows XP 不支援）。

7. **清理** — 在 Release 模式下，建置成功後會自動刪除中間的 `.obj` 和 `.res` 檔案。

### 前置需求

- **Visual Studio**（2017、2019、2022 或更新版本）需安裝「使用 C++ 的桌面開發」工作負載，或安裝 **Visual Studio Build Tools**。
- **Git** — 用於克隆 zstd 原始碼和初始化 vcpkg。
- **CMake** — 用於建置 zstd 靜態函式庫。

若要進行 Windows XP 相容的 x86 建置，必須安裝 **v141_xp 工具集**（可在 Visual Studio 2017 或 2019 中取得）。使用 `-Platform x86` 建置時，腳本會自動偵測並使用此工具集。

### 環境變數

如果第三方函式庫安裝在非標準位置，請在執行腳本前設定以下環境變數：

- `LIBYUV_ROOT` — libyuv 安裝根目錄路徑。
- `VPX_ROOT` — libvpx 安裝根目錄路徑。
- `SODIUM_ROOT` — libsodium 安裝根目錄路徑。
- `ZSTD_ROOT` — zstd 安裝根目錄路徑。

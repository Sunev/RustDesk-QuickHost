...
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
    ...
    set(LIBVPX_DIST_DIR_PREFIX "vpx-vp8-vp9")

    if(VCPKG_PLATFORM_TOOLSET STREQUAL "v141" AND VCPKG_PLATFORM_TOOLSET_VERSION MATCHES "^14\\.16")
        # XP-safe build: avoid the Vista+ worker implementation that pulls in
        # InitializeCriticalSectionEx / condition-variable APIs.
        set(OPTIONS "${OPTIONS} --disable-multithread")
        set(LIBVPX_DIST_DIR_PREFIX "${LIBVPX_DIST_DIR_PREFIX}-nomt")
    endif()
    ...
    set(LIBVPX_MSBUILD_OPTIONS)
    if(VCPKG_PLATFORM_TOOLSET STREQUAL "v141" AND VCPKG_PLATFORM_TOOLSET_VERSION MATCHES "^14\\.16")
        list(APPEND LIBVPX_MSBUILD_OPTIONS "/p:PlatformToolset=v141_xp")
    endif()

    vcpkg_msbuild_install(
        SOURCE_PATH "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-tmp"
        PROJECT_SUBPATH vpx.vcxproj
        OPTIONS ${LIBVPX_MSBUILD_OPTIONS}
    )

    if (VCPKG_TARGET_ARCHITECTURE STREQUAL arm64)
        set(LIBVPX_INCLUDE_DIR "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/${LIBVPX_DIST_DIR_PREFIX}-nopost-nodocs-${LIBVPX_TARGET_ARCH}${LIBVPX_CRT_SUFFIX}-${LIBVPX_TARGET_VS}-v${VERSION}/include/vpx")
    elseif (VCPKG_TARGET_ARCHITECTURE STREQUAL arm)
        set(LIBVPX_INCLUDE_DIR "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/${LIBVPX_DIST_DIR_PREFIX}-nopost-nodocs-${LIBVPX_TARGET_ARCH}${LIBVPX_CRT_SUFFIX}-${LIBVPX_TARGET_VS}-v${VERSION}/include/vpx")
    else()
        set(LIBVPX_INCLUDE_DIR "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/${LIBVPX_DIST_DIR_PREFIX}-nodocs-${LIBVPX_TARGET_ARCH}${LIBVPX_CRT_SUFFIX}-${LIBVPX_TARGET_VS}-v${VERSION}/include/vpx")
    endif()
    file(
        INSTALL
            "${LIBVPX_INCLUDE_DIR}"
